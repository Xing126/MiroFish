...
def get_simulation_history():
    ...
    try:
        ...
        # 优化：预先获取所有 report_id，避免在循环中重复扫描目录
        # 构建 simulation_id -> latest_report_id 的映射
        sim_to_report = {}
        reports_dir = os.path.join(os.path.dirname(__file__), '../../uploads/reports')
            ...
            for report_folder in os.listdir(reports_dir):
                ...
                try:
                    ...
                    if sid:
                        ...
                        created_at = meta.get("created_at", "")
                        # 只保留最新的 report
                        if sid not in sim_to_report or created_at > sim_to_report[sid]["created_at"]:
                            sim_to_report[sid] = {"report_id": rid, "created_at": created_at}
                except:
                    continue
        ...
        for sim in simulations:
            ...
            # 获取关联的 report_id（从预先构建的映射中查找）
            sim_dict["report_id"] = sim_to_report.get(sim.simulation_id, {}).get("report_id")
